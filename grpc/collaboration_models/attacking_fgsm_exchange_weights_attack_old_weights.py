...
versao = 'mesmo_parente_mesmo_dataset'
...
target_model = tf.keras.models.load_model('nids_sensor_A_versao_{}.keras'.format(versao))
...
class ModelBService(collaboration_pb2_grpc.NidsCollaborationServicer):
    ...
    def GetModelWeights(self, request, context):
        ...
        print("[GRPC] O modelo A está requisitando os pesos do modelo B")

        current_weights = self.model.get_weights()
        serialized_weights = pickle.dumps(current_weights)
        model_config = self.model.to_json()

        return collaboration_pb2.WeightResponse(
            model_weights=serialized_weights,
            architecture_json=model_config
                )
...
def apply_federated_averaging(model_a, stub):
    print('Solicitando pesos do modelo B')

    response = stub.GetModelWeights(collaboration_pb2.WeightRequest(request_message="Send weights"))

    weights_B = pickle.loads(response.model_weights)
    weights_A = model_a.get_weights()

    if len(weights_A) != len(weights_B):
        raise ValueError('Erro: quantidade diferente de pesos.')

    averaged_weights = []

    for w_a, w_b in zip(weights_A, weights_B):
        w_avg = (w_a + w_b) / 2.0
        averaged_weights.append(w_avg)

    model_a.set_weights(averaged_weights)
    print('Atualização concluida!')

print("\nIniciando FGSM")
print("Atacando modelo A cópia")
...
print('Atualizando pesos do modelo A')
...
epsilons = [0.001, 0.002, 0.003, 0.01, 0.02, 0.09, 0.1, 0.5, 0.9, 1.0, 1.1, 1.2, 1.3, 1.4, 1.5, 1.6]
